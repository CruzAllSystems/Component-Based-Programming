...
public class BingoCardRowSetBasedImpl_Chavez extends BingoCardRowSetBased_Abstract
{
	//STUDENT: DO *NOT* ADD ANY INSTANCE VARIABLES HERE OR IN THE ABSTRACT CLASS!!!

	public BingoCardRowSetBasedImpl_Chavez(Mystery_A mystery)
	{
		super(mystery);
	...
	public Integer getEntry(int row, int column)
	{
		...
		List<List<Integer>> columnListList = convertToColumnListList(rowSetList); 
	...
	//pre: none
	//post: returns the given rowSetList as a columnListList
	private List<List<Integer>> convertToColumnListList(List<Set<Integer>> rowSetList)
	{
		List<List<Integer>> columnListList = new ArrayList<List<Integer>>();
		List<List<Integer>> rowListList = new ArrayList<List<Integer>>();
		
		for(int index = 0; index < rowSetList.size(); index++)
		{
			List<Integer> currentRow = new ArrayList<Integer>(rowSetList.get(index));
			if(index == 2)
			{
				for(int index2 = 0; index2 < currentRow.size(); index2++)
				{
					if(currentRow.get(index2) == null)
						currentRow.remove(index2);
			...
			rowListList.add(currentRow);
			
			
		}//end of the rowListList making loop
	...
	}//end of the convertToCollumnListList helper method

	@Override
	public String toString()
	{
		return "Didn't get to test this one too thouroughly";
	}
}